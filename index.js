...
const ESCALACAO_CHANNEL_ID = process.env.ESCALACAO_CHANNEL_ID || "1510080509374632017";
const GERENTE_ACAO_ROLE_ID = process.env.GERENTE_ACAO_ROLE_ID;
...
const CONFIG = {
  ...
  gerenteAcaoRoleId: GERENTE_ACAO_ROLE_ID || "1509999055907066028",
  gerenteRoleId: "1509999055907066028",
...
async function isGerente(userId) {
  try {
    ...
    console.error("Erro ao verificar cargo de gerente:", error);
    return false;
  }
}
...
function escalacaoEmbed(escalacao) {
  ...
  const restante = participantes.length > 20 ? `\n... e mais ${participantes.length - 20}` : "";
  ...
  try {
    if (interaction.isChatInputCommand()) {
      if (interaction.commandName === "painel") {
        if (!(await isGerente(interaction.user.id))) {
          return interaction.reply({ content: "❌ Apenas a Gerência de Farme pode enviar o painel.", ephemeral: true });
        }

        const channel = await client.channels.fetch(CONFIG.painelChannelId).catch(() => null);

        if (!channel) {
          return interaction.reply({ content: "❌ Canal do painel não encontrado.", ephemeral: true });
        }

        await channel.send(buildPayload(painelEmbed(), [painelButton()], true));
      ...
      if (interaction.commandName === "painel_escalacao") {
        if (!(await isGerente(interaction.user.id))) {
          return interaction.reply({
            content: "❌ Apenas a Liderança/Gerência pode enviar o painel de escalação.",
            ephemeral: true
          });
        }
        ...
        await channel.send({
          ...buildPayload(painelEscalacaoEmbed(), [painelEscalacaoButton()], true)
        });
    ...
    if (interaction.isButton()) {
      ...
      if (interaction.customId === "iniciar_escalacao") {
        if (!(await isGerente(interaction.user.id))) {
          return interaction.reply({
            content: "❌ Apenas a Liderança/Gerência pode iniciar uma escalação.",
            ephemeral: true
          });
        }
      ...
      if (interaction.customId.startsWith("entrar_escalacao_")) {
        ...
        if (escalacao.participantes.includes(interaction.user.id)) {
          return interaction.followUp({ content: "⚠️ Você já está nessa escalação.", ephemeral: true }).catch(() => null);
        }

        if (escalacao.participantes.length >= escalacao.vagas) {
          return interaction.followUp({ content: "❌ Todas as vagas já foram preenchidas.", ephemeral: true }).catch(() => null);
        }
      ...
      if (interaction.customId.startsWith("win_escalacao_") || interaction.customId.startsWith("red_escalacao_")) {
        ...
        if (!membroTemCargoGerenteAcao(interaction)) {
        ...
        await interaction.message.edit({
          ...buildPayload(escalacaoEmbed(escalacao), [escalacaoButtons(escalacaoId, true)], false),
